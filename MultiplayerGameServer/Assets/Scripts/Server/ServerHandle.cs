...
namespace MultiplayerGameServer.Server
{
    public class ServerHandle
    {
        public static void WelcomeReceived(Packet packet, int fromClient = -1)
        {
            ...
            LogFactory.Instance.Info($"Player {Server.Instance.Clients[fromClient].TcpEndpoint} connected successfully and is player {message.ClientId}: {message.Username}");
            ...
            Server.Instance.Clients[fromClient].SpawnIntoGame(message.Username, message.ShipModelId);
        }
        ...
        public static void PlayerMovementReceived(Packet packet, int fromClient = -1)
        {
            ...
            Server.Instance.Clients[fromClient].Player?.SetMovement(movement);
        }

        public static void UdpTestReceived(Packet packet, int fromClient = -1)
        {
            UdpTest test = packet.ReadObj<UdpTest>();
            LogFactory.Instance.Info($"Player {fromClient} : {Server.Instance.Clients[fromClient].TcpEndpoint} connected successfully in udp: {test.Message}");
        }
    }
}
